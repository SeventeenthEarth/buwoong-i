...
def read_files_in_directory(directory: Union[str, os.PathLike], target_extension: str, title: str = None) -> str:
    if target_extension not in ['dart', 'py']:
        raise ValueError("Only 'dart' and 'py' file extensions are allowed.")
    ...
    for root, dirs, files in os.walk(abs_directory):
        ...
        dirs[:] = [d for d in dirs if d not in ['.venv', 'venv', '.idea', '.vscode', '__pycache__']]
        for file in files:
            if file.endswith(f".{target_extension}") and file != "__init__.py" and not file.endswith(".pyc"):
                target_files.append(os.path.join(root, file))
            elif file.lower() in ['dockerfile', 'docker-compose.yml', 'docker-compose.yaml', 'makefile']:
                infrastructure_files.append(os.path.join(root, file))

    # Add metadata section with file count and directory tree
    markdown_content += "## Metadata\n\n"
    markdown_content += f"- Total number of '{target_extension}' files: {len(target_files)}\n"
    markdown_content += f"- Total number of Infrastructure files: {len(infrastructure_files)}\n\n"
    markdown_content += "```\n"
...
def main(directory: str, extension: str, title: str = typer.Option(None, "--title", help="Title for the markdown output")):
